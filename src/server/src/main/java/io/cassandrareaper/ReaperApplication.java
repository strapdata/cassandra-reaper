...
public final class ReaperApplication extends Application<ReaperApplicationConfiguration> {
  ...
  private boolean selfRegisterClusterForSidecar(ClusterResource addClusterResource, String seedHost)
    ...
    final Cluster cluster = addClusterResource.findClusterWithSeedHost(seedHost, Optional.empty());
    if (null == cluster) {
      return false; // shutdown reaper? no point in starting side-car when C* is not there and running first…
    }

    if (context.storage.getClusters().stream().noneMatch(c -> c.getName().equals(cluster.getName()))) {
      LOG.info("registering new cluster : {}", cluster.getName());
